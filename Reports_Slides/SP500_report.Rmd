...
The difference in bevavior is quite visible. Moreover, we can see **different market regimes** during this period. 
...
names(mom_strategy)<-names(market)
```
...
Of course one could do this selection for shorter time windows to achieve even better returns. For example, these are the returns of the last 750 days if we do the selection using only data of these 750 days:
...
```
Although formally we need to de-mean the data in the calculations below, and also use a regressions constant ("alpha"), one could still ignore these mathematical formalisms and ignore these means and alpha - since in practice going forward one cannot assume these would remain constant. Afterall if we know the market (mean) returns in the future we would not need any of these analysis. We can assume all means and alphas are 0. 
```
...
pnl_plot(Stock_Residuals[,worst_stock_res])
```
