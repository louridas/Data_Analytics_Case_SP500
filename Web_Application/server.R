...
shinyServer(function(input, output,session) {
  ...
  output$histogram<-renderPlot({
    ...
    hist(values$thedata[input$start_date:input$end_date,],breaks=200)
    
  })
  
  output$stock_returns <- renderPlot({    
    stockx=values$thedata[input$start_date:input$end_date,][,input$ind_stock]
    names(stockx)<-rownames(values$thedata[input$start_date:input$end_date,])
    pnl_plot(stockx)    
  })
  
  output$market <- renderPlot({    
    market=apply(values$thedata[input$start_date:input$end_date,],1,mean)
    names(market)<-rownames(values$thedata[input$start_date:input$end_date,])
    pnl_plot(market)    
  })
  ...
  output$worst_stocks <- renderPlot({    
    best_stock=which.max(apply(values$thedata[input$start_date:input$end_date,],2,sum))
    worst_stock=which.min(apply(values$thedata[input$start_date:input$end_date,],2,sum))
    pnl_plot(values$thedata[input$start_date:input$end_date,][,worst_stock])
  })
  
  output$mr_strategy <- renderPlot({        
    market=apply(values$thedata[input$start_date:input$end_date,],1,mean)
    mr_strategy = -sign(shift(market,1))*market
    names(mr_strategy)<-rownames(values$thedata[input$start_date:input$end_date,])
    pnl_plot(mr_strategy)
  })
  
  output$eigen_plot <- renderPlot({    
    SP500PCA<-PCA(values$thedata[input$start_date:input$end_date,], graph=FALSE)
  ...
  output$eigen_returns <- renderPlot({   
    SP500PCA<-PCA(values$thedata[input$start_date:input$end_date,], graph=FALSE)
    PCA_component=ProjectData%*%norm1(SP500PCA_simple$vectors[,input$vector_plotted])
    if(sum(PCA_component)<0) {PCA_component=-PCA_component; flipped_sign=-1} else {flipped_sign=1}
    names(PCA_component)<-colnames(values$thedata[input$start_date:input$end_date,])
    pnl_plot(PCA_component)

    #component_weights=flipped_sign*norm1(SP500PCA_simple$vectors[,input$vector_plotted])
    #names(component_weights)<-colnames(values$thedata[input$start_date:input$end_date,])
    #plot(component_weights,ylab="Component Weights",xlab="Stock",main="Component Weights on Stocks")
