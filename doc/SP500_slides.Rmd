...
*Interactive chart:* Put the mouse on the plot to see daily values
...
int_market <- nvd3Plot(ReturnsSoFar ~ TheDate, data = data.frame.market, id = 'chart', type = 'lineChart')
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
data.frame.market<-data.frame(date=rownames(market)[1:4],rets=market[1:4,], row_names=rownames(market)[1:4] )
ecm <- reshape2::melt(data.frame.market[,c('date', 'rets')], id = 'date')
