...
## Averaging with Hindsight: Past `r round(nrow(ProjectData)/3)` days

Repeating the same but making the selection only using the performance of the past `r round(nrow(ProjectData)/3)` days
...
```{r results='asis'}
SP500PCA_simple<-eigen(cor(ProjectData))
TheFactors=apply(SP500PCA_simple$vectors[,1:numb_components_used],2,function(r) 
  if (sum(ProjectData%*%r)<0) -norm1(r) else norm1(r))
...
```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
pnl_plot(mr_Stock_Residuals[,best_stock_mr_res,drop=F])
```
