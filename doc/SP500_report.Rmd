...
Of course one could do this selection for shorter time windows to achieve even better returns. For example, these are the returns of the last `r round(nrow(ProjectData)/5)` days if we do the selection using only data of these `r round(nrow(ProjectData)/5)` days:

```{r echo=FALSE, comment=NA, warning=FALSE, message=FALSE,results='asis'}
recent_selected_strat=apply(tail(mr_ProjectData,round(nrow(ProjectData)/5)),2,function(r) if (sum(r)<0) -r else r)
recent_selected_mr_market=apply(recent_selected_strat,1,mean)
names(recent_selected_mr_market)<-names(tail(market,round(nrow(ProjectData)/5)))
...
```{r results='asis'}
SP500PCA_simple<-eigen(cor(ProjectData))
TheFactors=apply(SP500PCA_simple$vectors[,1:numb_components_used],2,function(r) if (sum(ProjectData%*%r)<0) -norm1(r) else norm1(r))
...
rownames(mr_Stock_Residuals)<-names(market)
...
rownames(mom_Stock_Residuals)<-names(market)
